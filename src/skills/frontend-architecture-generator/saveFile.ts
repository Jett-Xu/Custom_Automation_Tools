...
export const execute: SkillFunction = async (content: string) => {
  try {
    const targetDir = path.resolve(process.cwd(), "output/frontend/architecture");
    await fs.mkdir(targetDir, { recursive: true });
    
    const date = new Date();
    const timestamp = `${date.getFullYear()}${(date.getMonth() + 1).toString().padStart(2, "0")}${date.getDate().toString().padStart(2, "0")}_${date.getHours().toString().padStart(2, "0")}${date.getMinutes().toString().padStart(2, "0")}`;
    const fileName = `architecture_${timestamp}.md`;
    const filePath = path.join(targetDir, fileName);

    await fs.writeFile(filePath, content, "utf-8");

    return `✅ 架構生成處理完成！\n📁 存檔路徑：output/frontend/architecture/${fileName}`;
  } catch (error) {
